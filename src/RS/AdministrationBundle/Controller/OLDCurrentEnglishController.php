...
class CurrentEnglishController extends Controller
{
                ...
                for ($c=0; $c < $num; $c++) {
                    ...
                    if(isset($content->response->docs)) {
                        foreach ($content->response->docs as $doc) {
                            if(isset($doc->{'proxy_dc_type.en'})) {$dcType = $doc->{'proxy_dc_type.en'};}
                            elseif(isset($doc->{'proxy_dc_type.def'})) {$dcType = $doc->{'proxy_dc_type.def'};}
                            elseif(isset($doc->{'proxy_edm_type'})) {$dcType = $doc->{'proxy_edm_type'};}
                            else { $dcType = null;}

                            if(isset($doc->{'proxy_dc_subject.en'})) {$dcSubject = $doc->{'proxy_dc_subject.en'};}
                            ...
                            if(isset($doc->{'proxy_dc_creator.en'})) {$dcCreator = $doc->{'proxy_dc_creator.en'};}
