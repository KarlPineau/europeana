...
class ScoreComputingController extends Controller
{
    /*
        ...
        if (($handle = fopen($urlFile, "r")) !== FALSE) {
            while (($data = fgetcsv($handle, 0, ",")) !== FALSE) {
                ...
                if(isset($data[1]) AND isset($data[2]) AND isset($data[3]) AND isset($data[4]) AND isset($data[5]) AND isset($data[6]) AND isset($data[7])) {
                    ...
                    $listScores = [];
                    $response = $this->queryEuropeana($europeana_id, $dcType, $dcSubject, $dcCreator, $title, $dataProvider, $date, $timestamp, $when, $where, 1000, 0);
                    $content = $response[0];
